...
def clean_up_plot(fig, name, close_plot=True):
...
def load_CNT(full_path, rereference=True, set_channel_positions=True, adjust_annotations=True, crop_to_exp = True):
    ##################################
    ...
    def _crop_to_exp(raw):
        exp_boundaries = np.array([annot['onset'] for annot in raw.annotations if 'exp_' in annot['description']])
        raw.crop(*exp_boundaries)
